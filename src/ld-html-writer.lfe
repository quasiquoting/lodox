...
(defun format-docstring
  ([project _ m 'plaintext]
   (pre '(class "plaintext") (escape-html (mref m 'doc)))))
...
(defun topics-menu
  ([(= project `#m(documents ,docs)) current-doc] (when (is_list docs))
   `(,(h3 '(class "no-link") (span '(class "inner") "Topics"))
     ,(ul
      (lists:map (lambda (doc)
                   (li `(class ,(++ "depth-1" (if (=:= doc current-doc)
                                                " current"
                                                "")))
                        (link-to (doc-filename doc)
                          (div '(class "inner")
                           (span (escape-html (mref doc 'title)))))))
                  (lists:sort (lambda (a b) (=< (mref a 'name) (mref b 'name)))
                              docs))))))
                     ...
                     ([`#(,mod-name ,mod)]
                      (let ((class "depth-1")
                            (inner (div '(class "inner")
                                     (escape-html mod-name))))
                        (li `(class ,class)
                          (link-to (mod-filename mod) inner)))))
...
(defun project-title (project)
  (span '(class "project-title")
   `(,(span '(class "project-name") (escape-html (mref project 'name))) " "
     ,(span '(class "project-version") (escape-html (mref project 'version))))))

(defun header* (project)
  (div '(id "header")
   `(,(h2 `("Generated by " ,(link-to "https://github.com/quasiquoting/lodox" "Lodox")))
  ...
  (html
   (list
    (head
     (list
      (default-includes)
      (title (++ (escape-html (mref project 'name)) " "
                 (escape-html (mref project 'version))))))
    (body
     (list
      (header* project)
      ...
      (div '(id "content" class "namespace-index")
           (list
            ...
            (div '(class "doc") (p (escape-html (mref project 'description))))
            ;;   ;; TODO: package
            ;;   ;; TODO: topics
            (h2 "Modules")
            (lists:map
...
(defun document-page (project doc)
  (html
   (head
    (list
     (default-includes)
     (title (escape-html (mref doc 'title)))))
   (body
    (list
     (header project)
...
(defun func-docs (project module func)
  (div `(class "public anchor" id ,(escape-html (func-id func)))
       (list  (h3 (escape-html (mref func 'name)))
              ;; TODO: added and deprecated docs (?)
              (div '(class "usage")
                   (lists:map (lambda (form) (code (escape-html form))) (func-usage func)))
              (div '(class "doc")
  ...
  (html
   (list
    (head
     (list
      (default-includes)
      (title (++ (escape-html (mref module 'name)) " documentation"))))
    (body
     (list
      (header* project)
      ...
      (div '(id "content" class "namespace-docs")
           (list
            (h1 '(id "top" class "anchor") (escape-html (mref module 'name)))
